...
typedef struct {
    ...
    /* sample log patch by liwq */
    ngx_array_t                *sample_status;
    ngx_uint_t                  sample;
    ngx_uint_t                  skip_count;
...
/* sample log patch */
static ngx_int_t
ngx_http_sample_check_status(ngx_http_request_t *r, ngx_array_t *sample_st) {
    ngx_uint_t                  *tmp_st, i, st, key;
    ngx_http_variable_value_t   *var;

    static ngx_str_t s = ngx_string("status");

    if (sample_st == NULL || sample_st->nelts == 0) {
        return NGX_OK;
    }

    key = ngx_hash_key(s.data, s.len);
    var = ngx_http_get_variable(r, &s, key);
    if (1 != var->valid) {
        return NGX_ERROR;
    }
    st = ngx_atoi(var->data, var->len);
    if (st == 0) {
        return NGX_ERROR;
    }

    tmp_st = sample_st->elts;
    for (i = 0; i < sample_st->nelts; i++) {
        if (tmp_st[i] == st) {
            return NGX_OK;
        }
    }
...
ngx_http_log_handler(ngx_http_request_t *r)
{
    ...
    for (l = 0; l < lcf->logs->nelts; l++) {
        li = &log[l];
        /* sample log patch */
        if (0 < li->sample && ngx_http_sample_check_status(r, li->sample_status) == NGX_OK) {
            li->total_count += 1;
            if (li->skip_count * 100 < li->total_count * li->sample) {
                li->skip_count += 1;
                continue;
            }
            if (li->total_count > 100) {
                li->total_count = 0;
                li->skip_count = 0;
            }
        }
...
/* sample=10%:200,404 */
static ngx_int_t
ngx_http_log_set_sample(ngx_conf_t *cf, ngx_str_t *value, ngx_http_log_t *log) {
    u_char      *pb, *pe, *last;
    ngx_uint_t  *elt;
    ngx_uint_t  i = 10;

    pb = value->data + 7;
    last = (u_char *)ngx_strchr(pb, ' ');
    if (last == NULL) {
        last = value->data + value->len;
    }

    pe = (u_char *)ngx_strchr(pb, '%');
    if (pe == NULL) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "sample format error. \"%V\"", value);
        return NGX_ERROR;
    }

    log->sample = ngx_atoi(pb, pe-pb);
    if (log->sample > 100) {
        log->sample = 0;
    } else {
        log->sample = 100 - log->sample;
    }
    ...
    if (log->sample_status == NULL) {
        log->sample_status = ngx_array_create(cf->pool, 3, sizeof(ngx_uint_t));
        if (NULL == log->sample_status) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "param conf file ngx_array_create null");
            return NGX_ERROR;
        }
    }

    while (i--) {
        elt = ngx_array_push(log->sample_status);
        if (elt == NULL) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "param conf file ngx_array_push null");
            return NGX_ERROR;
        }

        pe = (u_char *)ngx_strchr(pb, ',');
        if (pe == NULL) {
            *elt = ngx_atoi(pb, last-pb);
#if 0
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "last status  \"%d\"", *elt);
#endif
...
process_formats:
    ...
    for (i = 3; i < cf->args->nelts; i++) {
        ...
        /* sample log patch */
        /* sample=10%:200,404 */
        if (ngx_strncmp(value[i].data, "sample=", 7) == 0) {
            if (NGX_OK != ngx_http_log_set_sample(cf, &value[i], log)) {
                return NGX_CONF_ERROR;
            }
            continue;
        }
